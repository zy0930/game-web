...
export default function TurnoverReportPage() {
  ...
  const { data: selectionsData, isLoading: isLoadingSelections } = useTurnoverGameSelections({
          ...
          <>
            ...
            <div className="mx-4 mb-4 bg-primary rounded-xl p-4">
              <div className="flex">
                <div className="flex-1 text-center border-r border-white/20">
                  ...
                  <p className="text-white font-roboto-bold">MYR {(recordsData?.TotalRollover ?? 0).toFixed(2)}</p>
                </div>
                <div className="flex-1 text-center">
                  <p className="text-white/80 text-xs mb-1">{t("report.totalWinLose")}</p>
                  <p className="text-white font-roboto-bold">MYR {(recordsData?.TotalWinLose ?? 0).toFixed(2)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="divide-y divide-zinc-100">
              {records.map((record) => {
                ...
                return (
                  <div key={record.Id} className="grid grid-cols-4 gap-2 px-4 py-3 text-xs">
                    <div className="text-zinc-600">
                      <div>{date}</div>
                      <div className="text-zinc-400">{time}</div>
                    </div>
                    <div className="text-zinc-800 flex items-center text-[10px]">{record.GameName}</div>
                    <div className="text-zinc-800 text-right flex items-center justify-end">{record.Rollover.toFixed(2)}</div>
                    <div className={cn(
